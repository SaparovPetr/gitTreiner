...
Ключевой идеей являяется возможность редактирования пользователем карточки к каждому изучаемому слову.

Сама карточка представлена в виде маркдаун файла, хранящегося в репозитории пользователя на Гитхабе.
...
Для быстрого создания заметки добавлена возможность ее генерации нейросетью DeepSeek.

Реализованнвый в этом приложении метод повторения нахожу для себя самым эфективным. По большому счету для этого приложение и было сделано.
...
По сути, приложение - это просто интерфейс для работы с собственной базой слов, которая хранится на Гитхабе и может стать доступной локально, если пользователь решит ее спулить.
...
- В настройках приложения пользователь может указать имя своего профиля и название ранее форкнутого репозитория.
...
- Находясь в карточке, пользователь может сгенерировать будущую заметку с помощью нейросети (указав свой апи-ключ).
...
- Если пользователь указал свой апи-ключ к DeepSeek, то сгенерированный ей ответ помещается в буфер обмена операционки и доступен для вставки в маркдаун-файл.
